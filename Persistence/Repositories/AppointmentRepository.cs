...
namespace PracticeManagement.Persistence.Repositories
{
    public class AppointmentRepository : IAppointmentRepository
    {
        ...
        /// <summary>
        /// Get appointments for single Profesor
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public IEnumerable<Appointment> GetAppointmentByProfesor(int id)
        {
            //return (from a in _context.Appointments where a.ProfesorId == id select a).AsEnumerable();
        ...
        /// <summary>
        /// Get upcomming appointments for Profesor - Admin section
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public IEnumerable<Appointment> GetTodaysAppointments(int id)
        {
            ...
            return _context.Appointments
                .Where(d => d.ProfesorId == id && d.StartDateTime >= today)
                .Include(p => p.Patient)
                .OrderBy(d => d.StartDateTime)
                .ToList();
        }
        /// <summary>
        /// Get upcomming appointments for specific Profesor
        /// </summary>
        ...
        public IEnumerable<Appointment> GetUpcommingAppointments(string userId)
        {
            DateTime today = DateTime.Now.Date;
            return _context.Appointments
                .Where(d => d.Profesor.PhysicianId == userId && d.StartDateTime >= today && d.Status==true)
                .Include(p => p.Patient)
                .OrderBy(d => d.StartDateTime)
                .ToList();
        }
        ...
        {
            var result = _context.Appointments.Include(p => p.Patient).Include(d => d.Profesor).AsQueryable();
            if (searchModel != null)
            {
                if (!string.IsNullOrWhiteSpace(searchModel.Name))
                    result = result.Where(a => a.Profesor.Name == searchModel.Name);
        ...
        {
            return _context.Appointments.Any(a => a.StartDateTime == appntDate && a.ProfesorId == id);
        }
        /// <summary>
